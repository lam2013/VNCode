...
from core import *
from lorem.lorem_gen import *
_lorem_cache = gen_lorem()  # pre-generate lúc khởi động, đảm bảo tức thì khi dùng

def _refresh_lorem_cache():
    """Regenerate lorem cache ngầm sau khi đã dùng, đảm bảo lần sau luôn tức thì."""
    global _lorem_cache
    _lorem_cache = gen_lorem()
...
class Main(QMainWindow):
    ...
    def initUI(self):
        ...
        QTimer.singleShot(900, self.check_value_file_path)

    def setProperty_tabfile_non_exist(self, index):
        widget = self.tabFile.widget(index)
        if widget and widget.property("non_exist") == False:
            self.tabFile.tabBar().setTabTextColor(index, QColor("#ffffff"))
            widget.setProperty("non_exist", None)
            return True
        return False

    def check_value_file_path(self):
        list_path_non_exist = []
        # loop số lượng tab trong tabFile
        for i in range(self.tabFile.count()):
            widget = self.tabFile.widget(i)
            #check widget có phải là none hay không
            if widget:
                #lấy giá trị "file_path" của widget được gắn cho editor
                path = widget.property("file_path")
                if path != "untitled" and path != None and Path(path).exists() == False:
                    list_path_non_exist.append(path)
        
        for path in list_path_non_exist:
            for i in range(self.tabFile.count()):
                widget = self.tabFile.widget(i)
                if widget:
                    if widget.property("file_path") == path:
                        self.tabFile.tabBar().setTabTextColor(i, QColor("#858585"))
                        widget.setProperty("non_exist", False)
    ...
    def create_editor(self, file_path=None, content=""):
        ...
        class CustomEditor(QPlainTextEdit):
            ...
            def keyPressEvent(self, event):
                ...
                # --- Lorem expansion: gõ "lorem" rồi Tab → sinh lorem ipsum ---
                if key_code == Qt.Key_Tab:
                    word = self.text_under_cursor().strip()
                    if word.lower() == "lorem":
                        global _lorem_cache
                        tc = self.textCursor()
                        tc.select(tc.WordUnderCursor)
                        tc.removeSelectedText()
                        tc.insertText(_lorem_cache)          # dùng cache → tức thì, 0ms
                        self.setTextCursor(tc)
                        event.accept()
                        QTimer.singleShot(0, _refresh_lorem_cache)  # refresh ngay sau khi insert
                        return
        ...
        try:
            if current_path:
                ...
                if file_path:
                    ...
                    return
            self.setProperty_tabfile_non_exist(idx)
            self.clean_label()
        except Exception as e:
    ...
    def _build_save_filter(self):
        parts = []
        for exts, lang in TYPE_FILE.items():
            ext_str = f"*{exts}"
