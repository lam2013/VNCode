...
# ═══════════════════════════════════════════════════════════════════════════
# PART 6: C# LSP Integration (csharp_lsp)
# ═══════════════════════════════════════════════════════════════════════════

class CSharpConnectionStatus(Enum):
    """C# connection status."""
    DISCONNECTED = "disconnected"
    CONNECTING = "connecting"
    CONNECTED = "connected"
...
class CSharpLspClient:
    """Client for C# Language Server (csharp-ls or OmniSharp)."""
    
    def __init__(self, server_path: str = "csharp-ls", project_root: str = "."):
        """
        Initialize C# LSP client.
        
        Args:
            server_path: Path to server executable (default: "csharp-ls")
            project_root: Root directory of the C# project
        """
        self.server_path = server_path
        self.project_root = project_root
        self.status = CSharpConnectionStatus.DISCONNECTED
    ...
    def start(self) -> bool:
        """
        Start the C# LSP process.
        
        Returns:
            True if started successfully, False otherwise
        """
        if self.status != CSharpConnectionStatus.DISCONNECTED:
            logger.warning(f"C# LSP already in state: {self.status.value}")
            return False
        
        try:
            self.status = CSharpConnectionStatus.CONNECTING
            
            # Resolve executable
            server_exe = self.server_path
            import shutil
            if not os.path.isabs(server_exe) and not shutil.which(server_exe):
                dotnet_tools_path = Path.home() / ".dotnet" / "tools"
                csharp_ls_local = dotnet_tools_path / "csharp-ls.exe"
                if csharp_ls_local.exists():
                    server_exe = str(csharp_ls_local)
                else:
                    csharp_ls_local_no_ext = dotnet_tools_path / "csharp-ls"
                    if csharp_ls_local_no_ext.exists():
                        server_exe = str(csharp_ls_local_no_ext)
            
            logger.info(f"Starting C# LSP from: {server_exe}")
            
            args = [server_exe]
            if "omnisharp" in server_exe.lower():
                args.append("-lsp")
            ...
            # Initialize
            if self.initialize():
                self.status = CSharpConnectionStatus.CONNECTED
                logger.info("C# LSP connected and initialized")
                return True
            else:
                self.stop()
                return False
            ...
            logger.error(f"Failed to start C# LSP: {e}")
            self.status = CSharpConnectionStatus.ERROR
            return False
    
    def stop(self) -> None:
        """Stop the C# LSP process."""
        if self.process:
            try:
                self.process.terminate()
                self.process.wait(timeout=5)
            except Exception as e:
                logger.error(f"Error stopping C# LSP: {e}")
                try:
                    self.process.kill()
                except:
                    pass
        ...
        logger.info("C# LSP stopped")
    
    def initialize(self) -> bool:
        """Initialize C# LSP with project info."""
        try:
            params = {
                "processId": os.getpid(),
                "rootPath": str(Path(self.project_root).absolute()),
            ...
            if response and "result" in response:
                self.initialized = True
                self.capabilities = response["result"].get("capabilities", {})
                logger.info("C# LSP initialized successfully")
            ...
            logger.error(f"C# LSP Initialization failed: {e}")
            return False
    
    def send_request(self, method: str, params: Dict[str, Any], timeout: float = 5.0) -> Optional[Dict]:
        """Send a request and wait for response."""
        if self.status != CSharpConnectionStatus.CONNECTED:
            logger.warning(f"C# LSP not connected: {self.status.value}")
            return None
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"C# LSP Request {method} timed out")
                return None
    ...
    def _read_responses(self) -> None:
        """Read responses from C# LSP process."""
        try:
            while self.process and self.process.stdout:
                # Read headers
                headers = {}
            ...
            logger.error(f"Error in C# LSP response reader: {e}")
        
        finally:
            self.status = CSharpConnectionStatus.DISCONNECTED
    
    def _handle_message(self, message: Dict) -> None:
        """Handle incoming message from C# LSP."""
        if "id" in message and message["id"] in self.pending_requests:
            self.pending_requests[message["id"]].put(message)
        elif "method" in message:
            method = message["method"]
            ...
            if method in self.notification_handlers:
                try:
                    ...
                    logger.error(f"Error handling C# LSP notification {method}: {e}")
    
    def register_notification_handler(self, method: str, handler: Callable) -> None:
        """Register a handler for C# LSP notifications."""
        self.notification_handlers[method] = handler
    
    def update_document(self, file_path: str, code: str) -> None:
        """Keep the document in sync with the LSP server."""
        if file_path not in self.opened_files:
            self.open_document(file_path, code)
    ...
    def open_document(self, file_path: str, content: str) -> None:
        """Notify C# LSP that a document was opened."""
        params = {
            "textDocument": {
                "uri": self._path_to_uri(file_path),
                "languageId": "csharp",
    ...
    def close_document(self, file_path: str) -> None:
        """Notify C# LSP that a document was closed."""
        params = {
            "textDocument": {"uri": self._path_to_uri(file_path)}
        }
        self.send_notification("textDocument/didClose", params)
    ...
    def get_completions(self, file_path: str, line: int, character: int, code: Optional[str] = None) -> List[Dict[str, Any]]:
        """Get completions from C# LSP."""
        if code is not None:
            self.update_document(file_path, code)
    ...
    def get_definition(self, file_path: str, line: int, character: int) -> Optional[Dict[str, Any]]:
        """Get definition location from C# LSP."""
        params = {
            "textDocument": {"uri": self._path_to_uri(file_path)},
            "position": {"line": line, "character": character}
        }
    ...
    def get_hover_info(self, file_path: str, line: int, character: int) -> Optional[str]:
        """Get hover information from C# LSP."""
        params = {
            "textDocument": {"uri": self._path_to_uri(file_path)},
            "position": {"line": line, "character": character}
        }
                ...
                if isinstance(content, dict):
                    ...
                    for item in content:
                        if isinstance(item, dict) and "value" in item:
                            processed_contents.append(item["value"])
...
_csharp_client: Optional[CSharpLspClient] = None

def get_csharp_client(server_path: str = "csharp-ls", project_root: str = ".") -> CSharpLspClient:
    """Get or create global C# LSP client."""
    global _csharp_client
    if _csharp_client is None:
        _csharp_client = CSharpLspClient(server_path, project_root)
    return _csharp_client


def start_csharp_lsp(server_path: str = "csharp-ls", project_root: str = ".") -> bool:
    """Start C# LSP server."""
    client = get_csharp_client(server_path, project_root)
    return client.start()
...
def get_csharp_lsp_completions(file_path: str, line: int, character: int, code: Optional[str] = None) -> List[Dict[str, Any]]:
    """Get completions from C# LSP."""
    client = get_csharp_client()
    if client.status == CSharpConnectionStatus.CONNECTED:
        return client.get_completions(file_path, line, character, code)
    return []


def get_csharp_lsp_definition(file_path: str, line: int, character: int) -> Optional[Dict[str, Any]]:
    """Get definition from C# LSP."""
    client = get_csharp_client()
    if client.status == CSharpConnectionStatus.CONNECTED:
        return client.get_definition(file_path, line, character)
    return None
...
def get_csharp_lsp_hover(file_path: str, line: int, character: int) -> Optional[str]:
    """Get hover info from C# LSP."""
    client = get_csharp_client()
    if client.status == CSharpConnectionStatus.CONNECTED:
        return client.get_hover_info(file_path, line, character)
    return None
